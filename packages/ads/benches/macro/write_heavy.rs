...
use generators::{uniform_keys, write_heavy_ops};

fn write_heavy_benches(c: &mut Criterion) {
    let mut group = c.benchmark_group("macro_write_heavy_u64");

    for &size in &[5_000usize, 20_000usize] {
        group.throughput(Throughput::Elements(size as u64));

        let prefill = uniform_keys(size, 0x6200 + size as u64);
        let ops = write_heavy_ops(size, size, 0x6300 + size as u64);
...
criterion_group!(macro_write_heavy, write_heavy_benches);
criterion_main!(macro_write_heavy);
